...
export function contract(){if(!deployment.contract)throw new Error("The arena contract is awaiting deployment.");return deployment.contract as `0x${string}`}
export async function readBounty(id:string):Promise<Bounty>{const raw=await client().readContract({address:contract(),functionName:"get_bounty",args:[id],transactionHashVariant:TransactionHashVariant.LATEST_FINAL});if(typeof raw!=="string")throw new Error("Invalid bounty response");return JSON.parse(raw)}
export async function listBounties(offset:number){if(!deployment.contract)return {bounties:[],hasMore:false};const ids=await client().readContract({address:contract(),functionName:"list_bounties",args:[offset,20],transactionHashVariant:TransactionHashVariant.LATEST_FINAL});if(!Array.isArray(ids))throw new Error("Invalid bounty list");return {bounties:await Promise.all(ids.map(id=>readBounty(String(id)))),hasMore:ids.length===20}}
