...
export async function write(session:Session,method:string,args:unknown[],value="0",stage:(message:string)=>void=()=>{},legacy=false){
 const address=legacy?legacyDeployment.contract:deployment.contract;
 if(legacy&&!["settle","claim_reward"].includes(method))throw new Error("This earlier arena is available for settlement and claims only.");
 ...
 const tx=await prepareWalletTransaction(walletTransaction(address,session.address,method,args,value));
 await walletRead(assertWallet(session));
