...
export async function connectWallet(w:WalletOption):Promise<Session>{
 ...
 const session={address:accounts[0] as `0x${string}`,provider:w.provider};await assertWallet(session);return session;
}
export async function assertWallet(s:Session){
 ...
 if(BigInt(await s.provider.request({method:"eth_chainId"}) as string)!==BigInt(studionet.id))throw new Error("Switch your wallet to GenLayer Studio before signing.");
}
