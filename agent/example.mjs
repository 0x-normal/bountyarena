...
import {submissionData,consensus} from "../lib/transaction.mjs";
...
const config=await request("/api/config");if(config.chainId!==61999||!/^0x[0-9a-fA-F]{40}$/.test(config.contract))throw new Error("Expected a deployed GenLayer Studio arena.");
...
if(contract.toLowerCase()!==config.contract.toLowerCase()||t.chainId!==61999||t.to.toLowerCase()!==consensus.toLowerCase()||BigInt(t.value)!==0n||t.data.toLowerCase()!==submissionData(config.contract,id,entry).toLowerCase())throw new Error("Prepared transaction differs from intended work.");
if(BigInt(t.gas)>5000000n||BigInt(t.gas)*BigInt(t.gasPrice)>10n**16n)throw new Error("Transaction exceeds the example agent's fee limit.");
