...
import {studioNext as studionet,nextRpc} from "../lib/network.mjs";
import {TransactionStatus,TransactionHashVariant} from "genlayer-js/types";
import {readFile,writeFile,mkdir} from "node:fs/promises";
import assert from "node:assert/strict";
import {transferAllocations} from "../lib/message-fees.mjs";
import {receiptState} from "../lib/receipt.ts";
const base="http://localhost:5173",deployment=JSON.parse(await readFile("artifacts/next-test-deployment.json","utf8"));
const contract=deployment.contract;
await mkdir(".keys",{recursive:true});await mkdir("artifacts",{recursive:true});
let run;try{run=JSON.parse(await readFile(".keys/next-live-test-state.json","utf8"))}catch{run={id:"next-"+Date.now(),steps:{},checks:[]}}
async function save(){await writeFile(".keys/next-live-test-state.json",JSON.stringify(run,null,2))}
...
const sponsor=await account("next-sponsor"),bad=await account("next-incomplete-agent"),good=await account("next-qualified-agent");
...
async function step(key,who,method,args,value=0n,expectFailure=false){
 let s=run.steps[key];if(s?.complete){if(s.fee_accounting){s.fees=receiptState({status:7,txExecutionResultName:"FINISHED_WITH_RETURN",data:{fee_accounting:s.fee_accounting}}).fees;await save()}return s;}
 if(!s){const messageAllocations=["claim_reward","settle"].includes(method)?transferAllocations(await read(args[0]),method):[];const estimate=await c(who).estimateTransactionFees({messageAllocations});if(estimate.feeValue>10n**18n)throw Error("Test fee exceeds cap");s={hash:await c(who).writeContract({address:contract,functionName:method,args,value,leaderOnly:false,fees:{distribution:estimate.distribution,feeValue:estimate.feeValue,messageAllocations}})};run.steps[key]=s;await save();console.log(key+" submitted "+s.hash)}
 const receipt=await client.waitForTransactionReceipt({hash:s.hash,waitUntil:"finalized",interval:4000,retries:40});
 ...
 s.complete=true;s.execution=state.state;s.fees=state.fees;s.fee_accounting=receipt.data?.fee_accounting;await save();console.log(key+" "+state.state);return s;
}
async function apiSubmit(key,who,evidence,summary){
 ...
 const receipt=await client.waitForTransactionReceipt({hash:s.hash,waitUntil:"finalized",interval:4000,retries:40});assert.equal(receiptState(receipt).state,"success");s.complete=true;await save();console.log(key+" finalized");
}
const root="https://raw.githubusercontent.com/genlayerlabs/genlayer-js/1b7f50a3a3f2963ea857941b0fb386081dd5c326/";
if(!run.funded){await request("/api/faucet",{wallet:sponsor.address});await request("/api/faucet",{wallet:bad.address});await request("/api/faucet",{wallet:good.address});run.funded=true;await save();console.log("Sponsor funded through app faucet")}
...
bounty=await read(run.id);assert.equal(bounty.winner,1);assert.equal(bounty.status,run.steps.claim?.complete?"paid":"awarded");
...
if(!run.balanceAfter){run.balanceAfter=String(await client.getBalance({address:good.address}));await save()}
const after=BigInt(run.balanceAfter);assert.equal(after-BigInt(run.balanceBefore)+BigInt(run.steps.claim.fees.consumed),100n);
...
const refundAfter=await client.getBalance({address:sponsor.address});assert.equal(refundAfter-BigInt(run.refundBalanceBefore)+BigInt(run.steps.refund.fees.consumed),50n);
...
const result={contract,bounty:run.id,sponsor:sponsor.address,incomplete_agent:bad.address,qualified_agent:good.address,reward_wei:"100",refund_wei:"50",checks:run.checks,transactions:Object.fromEntries(Object.entries(run.steps).map(([name,v])=>[name,{hash:v.hash,execution:v.execution,complete:v.complete,fees:v.fees}])),completed_at:run.completed_at};
await writeFile("docs/STUDIO_NEXT_TEST.json",JSON.stringify(result,null,2));console.log("LIVE STUDIO NEXT TESTS PASSED",JSON.stringify(result));
